...
...                 PracticeTypeServCatCheck
...                 ("Practice type/service category mismatch")
...
...                 Strategy:
...
# ─────────── Rule Config ───────────
${RULE_NAME}                PracticeTypeServCatCheck
${EXPECTED_ERROR_MSG}       Practice type/service category mismatch


*** Test Cases ***                                                            PROCESS_NAME
E2E Workflow For Practice Rule PracticeTypeServCatCheck Validation            ${PROCESS_PRACTICE}
    [Tags]    regression    negative    PracticeTypeServCatCheck    practice    BRG_0004
...
Execute Data Remediation E2E Workflow
    ...
    Validate PracticeTypeServCatCheck Rule Fired                   ${process_id_db}
...
Upload File And Get Tracking Number
    [Documentation]    Creates practice CSV with incompatible PracticeType
    ...                and Service Category inputs to trigger BRG_0004.
    [Arguments]    ${process_name}

    # ═══════════════════════════════════════════════════════════════
    # 🎯 FETCH LIVE DATA FROM SYMPLR DB
    # ═══════════════════════════════════════════════════════════════
    &{test_data}=    Get Practice Type Serv Cat Check Violation Data
    Log    &{test_data}
...
# RULE VALIDATION
# ═══════════════════════════════════════════════════════════════════
Validate PracticeTypeServCatCheck Rule Fired
    [Documentation]    Waits for DB sync and validates that the
    ...                PracticeTypeServCatCheck rule fired
